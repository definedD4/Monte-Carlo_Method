...
namespace Monte_Carlo_Method_3D.Calculation
{
    public class StCalculation : Calculation
    {
        ...
        public StCalculation(ICalculationConstraint constraint, int width, int height, GridData edgeData) : base(constraint)
        {
            m_Width = width;
            m_Height = height;
            m_EdgeData = edgeData;
        ...
        protected override GridData Simulate()
        {
            ...
            for(int x = 1; x < m_Width - 1; x++)
            {
                ...
                {
                    var sim = new StSimulator(new SimulationOptions(m_Width, m_Height, new IntPoint(x, y)));
