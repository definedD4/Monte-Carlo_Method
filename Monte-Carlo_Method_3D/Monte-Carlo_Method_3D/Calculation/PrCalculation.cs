...
namespace Monte_Carlo_Method_3D.Calculation
{
    public class PrCalculation : Calculation
    {
        ...
        public PrCalculation(ICalculationConstraint constraint, int width, int height, GridData edgeData, IEnumerable<GridIndex> calculationMask) : base(constraint, calculationMask)
        {
            m_Width = width;
            m_Height = height;
            m_EdgeData = edgeData;
        ...
        {
            var sim = new PrSimulator(new SimulationOptions(new GridSize(m_Height, m_Width), coords));
